...
```text
D:/CodeMyself/LightOverLeaf/out/packages/lite-20260912-163335-f86d42a2/LightOverLeaf-0.1.0-lite-win64.exe
大小：151883776 bytes
SHA-256：3FEBBBD0ECB887F9F5807CCAE02815D502A538B34CCAB0B7C0487ED51A037890
```

最终生产组合含真实 Windows SyncTeX Adapter 与独立 Unavailable Adapter；打包前和打包入口内的 `ctest` 均为 31/31 通过。内部 7z 归档测试通过，包含 5 个目录、268 个文件；通过 `Start-Process -Wait` 直接运行外层 EXE 的自动 smoke 模式，明确退出码为 0。单文件运行时解压到唯一临时目录，启动 `LightOverLeaf.exe`，退出后清理。
