...
修改：桌面构建完成后调用 scripts/runtime/deploy-miktex.ps1，从 CMakeCache 的 LIGHTOVERLEAF_THIRDPARTY_ROOT/miktex 读取安装树（可通过 -MiKTeXRoot 覆盖），增量复制至 EXE 旁 runtime/miktex。检查元数据、四个引擎/导航程序，并逐项核对源文件与目标文件大小。复制失败不报告构建成功。不执行下载、解压或测试；保留原生严格内置 Runtime 策略。
