...
int main(){int failures=0;auto check=[&](bool v,const char*m){if(!v){++failures;std::cerr<<m<<'\n';}};
 ...
 if(auto*s=std::get_if<std::shared_ptr<preferences::IKPreferencesStore>>(&ps)){auto app=preferences::createPreferences(*s);auto defaults=app->get();check(std::holds_alternative<preferences::KPreferences>(defaults)&&std::get<preferences::KPreferences>(defaults).m_engine=="pdflatex"&&std::get<preferences::KPreferences>(defaults).m_compileMode=="live","preferences defaults");
  auto saved=app->update({"D:/TeX","pdflatex",45000,"onSave"});check(std::holds_alternative<preferences::KPreferences>(saved)&&std::get<preferences::KPreferences>(saved).m_compileMode=="onSave","preferences save");}
